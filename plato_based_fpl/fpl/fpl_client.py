...
class Client(simple.Client):
    ...
    def configure(self) -> None:
        ...
        if self.datasource:
            ...
            self.aug_sampler = samplers_registry.get(self.aug_datasource, self.client_id)
            self.aigc_sampler = samplers_registry.aigc_get_no2(self.aigc_datasource, self.client_id) # iid-aigc_get_no, fpl-aigc_get_no2
            
            if self.exp_sample_num != 0:
                print("add aigc sampler")
    ...
    def _load_data(self) -> None:
        ...
        if (
            ...
            logging.info("[%s] Loading its local and aigc, augmented data source...", self)
            ...
            self.aug_datasource = datasources_registry.get(client_id=self.client_id, datasource_name = Config().data.aug_datasource)
    ...
    def _allocate_data(self) -> None:
        ...
        self.aug_trainset = self.aug_datasource.get_train_set()
    ...
    async def _train(self):
        ...
        try:
            ...
            training_time = self.trainer.train(self.trainset, self.aigc_trainset, self.aug_trainset, self.sampler, self.aigc_sampler, self.aug_sampler)
    ...
    async def _payload_to_arrive(self, response) -> None:
        ...
        self.dirichlet_propotion = [0]
        self.target_propotion = [0]
        self.exp_sample_num = 0
